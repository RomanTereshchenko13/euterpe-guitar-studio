...
const CHANGELOG = [
  { v:'2.15.0', date:'2026-08-03',
    ...
    uk:['Заняття. Скажи, скільки маєш часу — від п\'яти хвилин до двадцяти — і Euterpe збере те, що тобі справді час повторити, відлічить час, переведе з вправи у вправу, сама завершить і підсумує. Досі кожна вправа була нескінченним циклом, з якого ти виходив руками, а поняття «сьогоднішня практика» в застосунку не існувало.',
        '«Тренувати це» звідусіль. Кожен довідковий вид тепер має вихід в одне натискання до вправи саме про те, що в тебе на екрані: акорд — до акомпанементу вчасно, арпеджіо — до влучання в тони крізь зміни, гама — до повторення фраз, побудованих з неї, коло квінт — до чистих змін між акордами тональності. Цю обіцянку застосунок давав з самого початку, а тримав рівно в одному місці.',
        '«Джем над цим». Заграти під гармонію, що на екрані, коштувало п\'яти кроків — обрати тональність, обрати акорд, розкрити «Супровід», увімкнути бас і ударні, натиснути «Луп». Тепер це одна кнопка, поруч із панеллю, яка вже й так каже, що над цим грати.',
        'Прогрес, який щось розповідає. Замість п\'яти чисел за весь час — історія, яка в твоєї практики справді є: рядок на кожну вправу з останнім результатом, напрямком руху, особистим рекордом і — для вправ, що слухають, — найкращою похибкою тайминга в мілісекундах. Застосунок усе це зберігав і не показував нічого. Кожен рядок водночас — шлях назад у цю вправу.',
        'Один екран для кожної вправи. Усі дев\'ять малювалися під одним заголовком і жодного разу не казали, у якій ти вправі; тепер є єдина шапка вправи — вона називає її, тримає тональність, темп, мікрофон і вихід, а параметри згортає, щойно ти починаєш грати. Домівка практики починається з того, що робити далі, а не з десяти карток, і кожна картка позначена тим, що вона є: з оцінкою по мікрофону, з оцінкою за точністю або коуч.',
        'Одна навігація замість трьох смуг одна над одною, і гриф тепер стоїть над контролами, які його змінюють, а не під ними. Налаштування впорядковані за тим, що ти з ними робиш, а тюнер і вимірювання затримки зібрані в «Інструменти». На телефоні чотири розділи — нижня панель під великим пальцем.',
        'Чесні підписи скрізь: усі рядки, що обіцяли вже випущену можливість «згодом», прибрано. Три вправи отримали оцінку з мікрофоном у минулому випуску, а їхні власні описи про це не знали.'] },
  { v:'2.14.0', date:'2026-08-02',
    en:['Mic scoring reaches the Rhythm drills. Strumming & feel now grades the pattern you play — every strum is timed against the pattern\'s own slots, swing included, so a correctly swung player is not marked late. Over the changes grades the thing comping is actually about: landing each chord change in time. What you strum between the changes is your own rhythm and is left alone.',
    ...
    uk:['Оцінка з мікрофоном дійшла до ритм-вправ. «Бій і відчуття» тепер оцінює малюнок, який ти граєш: кожен удар звіряється з власними слотами малюнка — зі свінгом включно, тож зіграний зі свінгом не рахується як запізнення. «Крізь зміни» оцінює те, про що акомпанемент і є насправді: чи вчасно ти заходиш на кожну зміну акорду. Те, що ти граєш між змінами, — твій ритм, і його не чіпають.',
        'Виправлено важливе в рівні з оцінкою: з колонок мікрофон чує й сам застосунок, а що клацання заплановане рівно по сітці, воно лягає ідеально — тож прогон, у якому ти й не торкався гітари, міг повернутися як «Чітко, 32/32». Тепер Euterpe розпізнає надто машинно-рівний результат і чесно про це каже замість того, щоб намалювати приємну оцінку. Навушники — чисте вирішення, і кожна вправа з оцінкою тепер про це нагадує.',
        'У ритм-вправах з оцінкою гітара-підказка стихає — вона лягала рівно на ті долі, що вимірюються, тож застосунок оцінював би сам себе. Так і корисніше: мікрофон вимкнений — малюнок грають тобі, увімкнений — граєш ти. Клацання й гурт лишаються, щоб завжди було за що зачепитися.',
        'Один рушій оцінювання тепер обслуговує всі три вправи з оцінкою, тож виправлення в одній — виправлення в усіх.'] },
  { v:'2.13.0', date:'2026-08-02',
    en:['Euterpe can now hear you play. Turn the microphone on in Subdivision & timing and the drill stops being a metronome you follow and becomes one that grades you: every note you pick is timed against the grid, and at the end you get your average error in milliseconds, how tight your spread is, and how many grid slots you actually hit.',
    ...
    uk:['Euterpe тепер чує, як ти граєш. Увімкни мікрофон у вправі «Поділ і тайминг» — і вона перестає бути метрономом, за яким ти йдеш, і стає таким, що тебе оцінює: кожна зіграна нота звіряється з сіткою, а наприкінці ти бачиш середню похибку в мілісекундах, наскільки щільний розкид і скільки клітинок сітки ти справді влучив.',
        'Вправа також каже, *у який бік* ти відхиляєшся — поспішаєш чи запізнюєшся, — але лише коли нахил справжній. Якщо ноти просто розкидані, а не стабільно раніше чи пізніше, вона так і скаже, замість вигадувати тенденцію з шуму. Вправа, що тобі лестить, — не коуч.',
        'Нове в «Налаштуваннях ▸ Інструмент»: затримка звуку. Пристрою потрібен час, щоб випустити звук з динаміка й повернути його через мікрофон, і без цього виміру будь-якому гітаристу світу казали б, що він запізнюється. Натисни «Виміряти», кілька секунд не чіпай гітару й дай йому поклацати — він грає клац, слухає його й бере середину з кількох замірів, щоб випадковий шум не зіпсував результат. У навушниках мікрофону нічого чути, тож він чесно про це скаже й лишить ручний повзунок.',
        'Тайминг, за яким тебе оцінюють, міряється на аудіопотоці, а не на кадрі анімації, тож число описує зіграну ноту, а не завантаженість браузера. Без мікрофона вправа працює точно як раніше — коуч-рівень не змінився, рівень з оцінкою вмикається за бажанням.',
        'Як і в тюнері, звук нікуди не йде з браузера: мікрофон працює лише під час прогону з оцінкою чи виміру затримки, і нічого не записується й нікуди не надсилається.'] },
  { v:'2.12.0', date:'2026-08-01',
    en:['New: a microphone tuner. Play any note on the guitar and see which note it is and how far off you are, on a needle that reads up to 50 cents sharp or flat — plus the nearest open string of whatever tuning you are in, so Drop D and the rest re-label themselves. It is chromatic, so you can also check your intonation at the 12th fret, not just tune the open strings. Open it with the Mic button next to the Tuner in Settings.',
    ...
    uk:['Нове: тюнер із мікрофоном. Зіграй будь-яку ноту на гітарі — і побачиш, що це за нота й наскільки ти відхилився, на шкалі до 50 центів вгору чи вниз, а ще найближчу відкриту струну твого поточного строю, тож Drop D та інші перепозначаються самі. Тюнер хроматичний, тож ним можна перевірити й інтонацію на 12-му ладі, а не лише налаштувати відкриті струни. Відкривається кнопкою «Мікрофон» поруч із «Тюнером» у «Налаштуваннях».',
        'Старий тюнер з еталонним тоном лишився без змін — той грає висоту, щоб ти підстроївся на слух, а новий слухає тебе. Обирай, що зручніше в конкретній кімнаті; тюнер із мікрофоном потребує мікрофона й захищеного зʼєднання (https) і тихо ховається, коли працювати не може, — щоб на екрані не було кнопок, які нічого не роблять.',
        'Звук нікуди не йде з браузера: мікрофон увімкнений лише поки відкрита панель тюнера, вимикається щойно ти її закриваєш або перемикаєш вкладку, і нічого не надсилається й не записується.'] },
  { v:'2.11.0', date:'2026-08-01',
    en:['New Foundations drill — Subdivision & timing: a smart visual metronome for locking in your time. Pick a subdivision (quarter notes → eighths → triplets → sixteenths) and a tempo; an accented click and a grid tick out the bar (a strong beat 1, softer subdivisions), and the current key\'s scale is walked note-by-note across the grid in one neck box so you always have something to play. Change key, box or tempo without leaving Practice. A coach that serves both the Rhythm and Lead pillars — mic-scored timing arrives with microphone input later.',
    ...
    uk:['Нова вправа-основа — «Поділ і тайминг»: розумний візуальний метроном, щоб тримати час. Обери поділ (чвертки → вісімки → тріолі → шістнадцятки) і темп; акцентоване клацання й сітка відбивають такт (сильний «раз», тихіші поділи), а гама поточної тональності «прокрокує» по сітці нота за нотою в одній позиції на грифі — тож завжди є що грати. Змінюй тональність, позицію чи темп, не виходячи з «Практики». Коуч, що працює і на ритм-, і на соло-пілон — оцінку тайминга з мікрофоном додамо згодом.',
        'Розміри такту: у «Налаштуваннях» зʼявився вибір розміру (2/4, 3/4, 4/4, 6/8, 12/8). Акцент метронома, грув супроводу (малюнок бочки/малого), бас, луп одного акорду й довжина такту всієї прогресії / акомпанементу / прицілювання — усе слідує за розміром, тож можна зіграти вальс у 3/4 чи відчуття 6/8, а не лише 4/4.',
        'Виправлено: якщо була відкрита вправа «Поділ і тайминг», повторне натискання «Практики» малювало список вправ поверх тієї, що ще працювала. Тепер перемикання режимів однаково опрацьовує всі вправи.',
        'Легший застосунок: архів нотаток до випусків більше не вантажиться зі сторінкою — у застосунку лишились найновіші випуски, а повна історія в CHANGELOG.md — і зник регулятор зсуву часу, який ще нічим не використовувався. Завантаження приблизно на 15% менше.',
        '«Налаштування» тепер стартують згорнутими й читаються як три групи — Інструмент, Гриф і розмір, Застосунок — замість одинадцяти контролів в один ряд. Палітра для дальтоніків і форми на нотах переїхали до налаштувань застосунку, де й місце разовим уподобанням.',
        '«Малюнки боя» і «Грув і відчуття» тепер одна вправа — «Бій і відчуття»: обирай і малюнок, і відчуття — свінг, бекбіт-акцент, глушіння та гурт (барабани з басом). Поєднання, недосяжні раніше в жодній із них (фолк-малюнок зі свінгом, приглушений бій під гурт), тепер просто працюють.',
        '«Акомпанемент прогресії» і «Тони акорду в прогресії» тепер одна вправа з перемикачем «Що граєш»: «Акорди» (акомпануй, з формами у «Зараз/Далі») або «Тони акорду» (влучай у тони на грифі, з позицією та ціллю). Обидві картки в «Практиці» відкривають її — кожна у своєму режимі, а збережений прогрес для обох лишився без змін.',
        'Усі вправи тепер мають один спільний вибір тональності й одну кнопку «Вийти» вгорі «Практики» замість власної копії в кожній. А вибір тональності зʼявляється лише для вправ, які його справді використовують — у вправах на слух і в назвах нот більше немає контролю, що нічого не змінює.',
        '«Слух» більше не окремий режим: три вправи на слух переїхали в «Практику» окремою групою «Слух» — поруч із «Грифом», «Таймингом», «Ритмом» і «Соло». Один список вправ, одна картка прогресу, один «Вийти» — за обома екранами й так стояла та сама модель навчання. Увесь збережений прогрес лишився, а старе посилання чи сесія, що відкривали «Слух», тепер відкривають «Практику».',
        'Значно менше завантаження: сторінка більше не вантажить коментарі розробника, написані для тих, хто редагує вихідний код. Разом із попередніми скороченнями сторінка приблизно на 36% менша, ніж у минулому випуску, і приблизно на третину менша «по дротах».'] },
