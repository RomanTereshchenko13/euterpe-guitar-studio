/* ============================ VENDORED THIRD-PARTY CODE ============================
   pitchy v4.1.0 — https://github.com/ianprime0509/pitchy
   Copyright (c) Ian Johnson. Permissively licensed (npm metadata says MIT; the
   repo LICENSE carries the ISC/0BSD grant text). Either way: permissive, no
   copyleft, so it cannot relicense this app's single-file output.

   THIS IS NOT OUR CODE. Do not "improve", reformat or refactor it — the only
   sanctioned change is replacing it wholesale with a newer upstream release
   (re-apply the three deltas below). Bug reports go upstream, not here.

   Why it's here: it implements the McLeod Pitch Method (MPM) — the NSDF, key-
   maximum picking and parabolic peak refinement from McLeod & Wyvill's "A Smarter
   Way to Find Pitch". That is the genuinely subtle, already-solved problem the
   dependency policy exists to let us borrow instead of re-deriving badly; a tuner
   that confidently reports the wrong octave is worse than no tuner. Per the
   policy the source is COPIED IN and concatenated by build.js, so the shipped app
   still fetches nothing at runtime and has no supply-chain surface.

   NOTE — the roadmap called this "the one code dependency, 0BSD". Both halves were
   slightly off: it is permissive-but-not-0BSD (above), and it is not alone — it
   needs fft.js, vendored alongside in 00-vendor-fft.js. Two libraries, both MIT-
   class, both audited here.

   Deltas from upstream index.js (kept to the absolute minimum):
     1. `import FFT from "fft.js";` removed — FFT comes from the vendored
        00-vendor-fft.js, which sorts ahead of this file into the bundle.
     2. `export class` → `class` on Autocorrelator and PitchDetector — there is no
        module system in the single concatenated <script>.
     3. Wrapped in an IIFE returning PitchDetector (see 00-vendor-fft.js for why
        the wrapper is load-bearing: it keeps `'use strict'` and the file's
        helpers off the shared app scope). Autocorrelator stays internal — it is
        upstream API we don't call, so it isn't lifted to the shared scope.
   ================================================================================= */
