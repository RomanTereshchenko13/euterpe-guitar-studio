...
- Fixed: with the Subdivision & timing drill open, tapping "Practice" again drew the drill list on top of the still-running drill. Switching modes now handles every drill the same way.
- Leaner app: the release-notes archive no longer ships with the page — only the newest releases are in-app, and the full history lives in CHANGELOG.md — and the timing-calibration control (which nothing used yet) is gone. The download is about 15% smaller.
